...
impl Axis {
    ...
    pub const fn as_flip(self) -> Flip {
        const _SAFETY: () = {
            ...
            isit::const_assert(1 << (Axis::X as u8) == Flip::X.as_u8());
            isit::const_assert(1 << (Axis::Y as u8) == Flip::Y.as_u8());
            isit::const_assert(1 << (Axis::Z as u8) == Flip::Z.as_u8());
        };
        // SAFETY: Flip is represented using the first 3 bits of the byte.
        //         Axis is represented as `X = 0, Y = 1, Z = 2`.
        //         The index of the bits in `Flip` correspond to the discriminant value of `Axis.`
        //         If the program compiles (the above checks passed), then this code is sound.
        unsafe {
